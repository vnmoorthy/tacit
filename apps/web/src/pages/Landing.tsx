    ...
    <div className="min-h-screen bg-paper text-ink">
      ...
      <header className={cx("sticky top-0 z-40 transition", scrolled ? "border-b border-line bg-paper/85 backdrop-blur" : "")}>
        <div className="mx-auto flex max-w-[1180px] items-center gap-6 px-5 py-4 md:px-8">
          ...
          <div className="ml-auto flex items-center gap-2">
            <a href={REPO} target="_blank" rel="noreferrer" className="hidden md:block">
              <Button size="sm" variant="ghost" icon={<Github className="h-4 w-4" />}>
                GitHub
              </Button>
            </a>
            <Link to="/app">
        ...
        <div className="mx-auto grid max-w-[1180px] items-center gap-10 px-5 pb-16 pt-12 md:grid-cols-[1.2fr_1fr] md:px-8 md:pb-24 md:pt-20">
          <div className="rise-in">
            ...
            <p className="mt-6 max-w-xl text-[17px] leading-relaxed text-ink-2 md:text-[19px]">
              A voice AI that interviews your departing experts and turns what's in their head into a living, cited knowledge base their successor can talk to. In their own words. In their own voice.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/app">
                <Button variant="accent" size="lg" icon={<Sparkles className="h-4 w-4" />}>
                  Try it now — no sign-up
                </Button>
              </Link>
              <a href="#demo">
                <Button size="lg" icon={<Play className="h-4 w-4" />}>
                  Watch the 2-minute demo
                </Button>
              </a>
            </div>
            <p className="mt-4 text-[13px] text-muted">Runs in your browser with zero keys. Add Boson + Nebius keys for real-time voice and a frontier model.</p>
          </div>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {[
            ...
            <div key={n} className="rounded-xl border border-line bg-paper-2/70 p-6">
              <div className="font-display text-[48px] leading-none text-accent">{n}</div>
              <p className="mt-3 text-[15px] leading-snug text-ink-2">{t}</p>
              <p className="mt-3 text-[12px] text-muted">Source: {src}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1180px] px-5 py-16 md:px-8 md:py-24">
          ...
          <div className="mt-12 grid gap-4 md:grid-cols-4">
            {[
              ...
              { n: "04", icon: <ArrowRight className="h-5 w-5" />, t: "Close the gap", b: "Anything the twin can't answer is queued, and becomes the first question of the next interview. The coverage map shows exactly what's still missing." },
            ].map((s) => (
      ...
      <section id="product" className="mx-auto max-w-[1180px] px-5 py-16 md:px-8 md:py-24">
          ...
          {[
            {
              ...
              b: "One question at a time. Atoms appear on the right as the expert speaks and the coverage map fills in live, steering the interviewer toward what's still missing. Voice on Higgs Realtime, browser voice, or typing: the demo can't die on stage.",
              pts: ["Semantic turn-taking and barge-in", "Follow-ups on \"usually\", \"depends\", \"the trick is\"", "Interviews in 18 languages; atoms always in English"],
            },
            {
              img: "/img/ask.jpg",
              t: "Ask the twin",
              b: "The successor asks. The twin answers only from what the expert said, with numbered citations back to the transcript, spoken in the expert's cloned voice. Low confidence? The question is queued for the expert.",
              pts: ["Hybrid retrieval: BM25 + embeddings", "Every claim cited to an atom", "Unanswered questions feed the next interview"],
              flip: true,
            },
            {
              img: "/img/graph-answer.jpg",
              t: "The Constellation",
            ...
            <div key={f.t} className={cx("grid items-center gap-8 md:grid-cols-2 md:gap-14", f.flip && "md:[&>*:first-child]:order-2")}>
              <img src={f.img} alt={f.t} className="w-full rounded-xl border border-line-2 shadow-lift" loading="lazy" />
              <div>
        ...
        <div className="mt-20 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[
            [<Mic className="h-4 w-4" />, "Real-time voice interviews", "Speech-to-speech on Higgs Realtime, ~0.7 s, interruptions handled."],
            [<Brain className="h-4 w-4" />, "Live extraction", "Nine atom types with confidence and source quotes, while the expert talks."],
            [<ShieldCheck className="h-4 w-4" />, "Coverage map", "Planned domains × target questions. Know what a successor still can't do."],
            [<Volume2 className="h-4 w-4" />, "The expert's own voice", "Higgs Audio clones the voice from the interview, with consent."],
            [<Globe2 className="h-4 w-4" />, "Any language", "Interview in Spanish, Tamil or Mandarin; the knowledge base stays in English."],
      ...
      <section id="uses" className="border-y border-line bg-paper-2/60">
        <div className="mx-auto max-w-[1180px] px-5 py-16 md:px-8 md:py-24">
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              [<Users className="h-5 w-5" />, "The retirement wave", "Peak 65 means four million retirements a year. Capture the 20-year veteran before their last day, and hand their successor a twin instead of a binder."],
              ...
              [<Hospital className="h-5 w-5" />, "Clinical and care teams", "Charge nurses, lab managers and clinic coordinators carry the workarounds that keep patients safe. Interview in their language; keep the atoms in English."],
              ...
              [<Landmark className="h-5 w-5" />, "Regulated and public sector", "Banks and agencies must retain key-person knowledge. Cited, verifiable atoms and a handover document satisfy the audit."],
      ...
      <section id="demo" className="mx-auto max-w-[1180px] px-5 py-16 md:px-8 md:py-24">
        <div className="grid gap-8 md:grid-cols-[1fr_1.6fr] md:items-center">
          <div>
            ...
            <p className="mt-4 text-[16px] leading-relaxed text-ink-2">Meet Maria, a payroll lead leaving in 42 days. Watch an interview turn into atoms, her successor ask the twin, and the constellation light up. Narrated by Higgs Audio.</p>
            <Link to="/app" className="mt-6 inline-block">
              <Button variant="primary" size="lg" icon={<ArrowRight className="h-4 w-4" />}>
                Open Maria's capture
              </Button>
            </Link>
          </div>
          <video controls preload="metadata" poster="/img/graph-answer.jpg" className="w-full rounded-xl border border-line-2 bg-black shadow-lift" src={VIDEO} />
        </div>
      </section>
          ...
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a href={REPO} target="_blank" rel="noreferrer">
                <Button variant="accent" size="lg" icon={<Github className="h-4 w-4" />}>
                  Star on GitHub
                </Button>
              </a>
              <Link to="/app">
...
# optional, for real-time voice + a frontier model
